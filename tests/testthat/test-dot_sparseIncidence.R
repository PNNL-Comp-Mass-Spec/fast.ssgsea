...
test_that("x is a named list of character vectors", {
   err1 <- capture_error(
      .sparseIncidence(gene_sets = letters,
                       background = "a")
   )$message

   err2 <- capture_error(
      .sparseIncidence(gene_sets = list("Set1" = 1:3),
                       background = "a")
   )$message

   expected_err <- "`gene_sets` must be a named list of character vectors."
...
test_that("the sets overlap with the background", {
   ...
   err <- capture_error(
      .sparseIncidence(gene_sets = list("Set1" = letters),
                       background = LETTERS)
   )$message

   expect_identical(
      object = err,
      expected = "No elements of `gene_sets` are present in rownames(X)."
   )
})
...
test_that("The directional incidence matrix is correct", {
   ...
   err <- capture_error(
      fast.ssgsea:::.sparseIncidence(gene_sets = gene_sets,
                                     background = background),
   )$message
