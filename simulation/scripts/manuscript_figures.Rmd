...
pb <- filter(desktop, nperm == "10,000") %>%
  plot_time(y_breaks = seq(0, 1440, 240))
...
pb <- filter(desktop_fast, nperm == "10,000") %>%
  plot_time(y_breaks = seq(0, 120, 30))
...
ggsave(
  filename = "../figures/supp-figure-1.pdf",
...
pa <- ggplot(df, aes(x = set_size, y = NES.base)) +
  ...
  geom_text(
    aes(
      x = 500,
      y = upper.base + 1,
      label = round(upper.base, digits = 2L)
    ),
    color = "red3",
    size = 8 / .pt,
    data = confint_df
  ...
  geom_text(
    aes(
      x = 500,
      y = lower.base - 1,
      label = round(lower.base, digits = 2L)
    ),
    color = "red3",
    size = 8 / .pt,
    data = confint_df
...
pb <- ggplot(df, aes(x = set_size, y = NES.fast)) +
  ...
  geom_text(
    aes(
      x = 500,
      y = upper.fast + 1,
      label = round(upper.fast, digits = 2L)
    ),
    color = "red3",
    size = 8 / .pt,
    data = confint_df
  ...
  geom_text(
    aes(
      x = 500,
      y = lower.fast - 1,
      label = round(lower.fast, digits = 2L)
    ),
    color = "red3",
    size = 8 / .pt,
    data = confint_df
...
ggsave(
  filename = "../figures/supp-figure-2.pdf",
...
h1 <- pval_hist(
  pval = df$p_value.base[grepl("0", df$alpha)],
...
h2 <- pval_hist(
  pval = df$p_value.base[grepl("1", df$alpha)],
...
h3 <- pval_hist(
  pval = df$p_value.fast[grepl("0", df$alpha)],
  alpha = df$alpha[grepl("0", df$alpha)],
  title = "fast-ssGSEA",
  ylim = hist_ylim
)
...
h4 <- pval_hist(
  pval = df$p_value.fast[grepl("1", df$alpha)],
  alpha = df$alpha[grepl("1", df$alpha)],
  title = "fast-ssGSEA",
  ylim = hist_ylim
)
...
```{r tab-1, include=FALSE, eval=FALSE}
# Tables {.unnumbered}
...
table_1 <-
  c(
    "../data/base-ssGSEA_timing_results_for_comparison.rds",
    "../data/fast-ssGSEA_timing_results_BLAS_for_comparison.rds"
  ) %>%
  lapply(function(file_i) {
    readRDS(file_i) %>%
      summarise(
        .by = -c(replicate, elapsed_time),
  ...
  mutate(
    n_signif_0 = pmax(floor(log10(floor(fast_0))) + 2L, 1L),
...
```{r tab-s1, include=FALSE, eval=FALSE}
...
table_1 <-
  ...
  setNames(c("base", "fast_BLAS", "fast_OpenBLAS")) %>%
  bind_rows(.id = "method") %>%
  arrange(nperm, nSamples, nSets, maxSetSize, method, alpha) %>%
  tidyr::pivot_wider(
    names_from = c(method, alpha),
  ...
  mutate(
    # BLAS
    n_signif_BLAS_0 = pmax(
      floor(log10(floor(fast_BLAS_0))) + 2L,
      1L
    ),
    n_signif_BLAS_1 = pmax(
      floor(log10(floor(fast_BLAS_1))) + 2L,
      1L
    ),
    ratio_BLAS_0 = format_ratio(base_0 / fast_BLAS_0,
      digits = n_signif_BLAS_0
    ),
    ratio_BLAS_1 = format_ratio(base_1 / fast_BLAS_1,
      digits = n_signif_BLAS_1
    ),
    # OpenBLAS
    n_signif_OpenBLAS_0 = pmax(
      floor(log10(floor(fast_OpenBLAS_0))) + 2L,
      1L
    ),
    n_signif_OpenBLAS_1 = pmax(
      floor(log10(floor(fast_OpenBLAS_1))) + 2L,
      1L
    ),
    ratio_OpenBLAS_0 = format_ratio(base_0 / fast_OpenBLAS_0,
      digits = n_signif_OpenBLAS_0
    ),
    ratio_OpenBLAS_1 = format_ratio(base_1 / fast_OpenBLAS_1,
      digits = n_signif_OpenBLAS_1
    )
  ) %>%
  select(-contains("n_signif"))
