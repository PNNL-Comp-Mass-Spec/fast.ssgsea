...
fast-ssGSEA is based on Post-Translational Modification Signature Enrichment Analysis (PTM-SEA) [@krug-curated-2019], and it borrows optimization techniques from the simple implementation of Fast Gene Set Enrichment Analysis (FGSEA-simple) [@korotkevich-fast-2021].
...
A macOS binary is provided in the [latest release](https://github.com/pnnl/fast.ssgsea/releases). Users looking to build and install the development version of `fast.ssgsea` must have the Xcode developer tools from Apple. See https://mac.r-project.org/tools/ for instructions.
...
This shows the runtime of `fast_ssgsea` on an AMD Ryzen 5 7600X CPU with a clock speed of 4.7 GHz. A total of 1 million permutations were used to calculate P-values and normalized enrichment scores (NES).
